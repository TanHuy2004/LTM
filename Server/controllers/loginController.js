...
const LoginController = {
  async login(req, res) {
    try {
      ...
      const token = jwt.sign({ id: user.ID_Taikhoan, role: user.VaiTro || "User" }, SECRET, { expiresIn: "2h" });
      ...
      return res.status(500).json({ success: false, message: "Lỗi server, không thể đăng nhập!", error: error.message });
    }
  },
};
